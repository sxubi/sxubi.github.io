...
**Definition** (Field)**.** A *field* is a non-zero ring where $$u\neq 0_R\in R$$ is a unit.     
...
**Proposition** (Euclidean alorithm for polynomials)**.** Let $$\mathcal{F}$$ be a field and $$f,g\in\mathcal{F}[X]$$. Then there is some $$r,q\in \mathcal{F}[X]$$ such that
