...
> * Those Mean-Value theorems could be used to prove the convergence of sequence, e.g. if $$\displaystyle a_n=n\tan^{-1}\left(\frac{1}{n}\right)$$, consider $$f(x)=n\tan^{-1}\left(\frac{x}{n}\right)$$.
> * When involving limits, remember to use the definition of limits, e.g. if $$\displaystyl\lim_{x\to\infty}F(x)=0$$, consider take an $$\varepsilon$$ and use $$x>M$$ to narrow the interval.
