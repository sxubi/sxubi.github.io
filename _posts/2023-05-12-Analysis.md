...
▲**Theorem (Absolute Convergence Test).** If $$\displaystyle\sum_{k=1}^\infty|a_k|$$ converges, then $$\displaystyle\sum_{k=1}^\infty a_k$$ converges. 

**Definition.** $$\displaystyle\sum_{k=1}^\infty a_k$$ converges absolutely iff $$\displaystyle\sum_{k=1}^\infty a_k$$ converges conditionally iff $$\displaystyle\sum_{k=1}^\infty a_k$$ converges but $$\displaystyle\sum_{k=1}^\infty |a_k|$$ diverges.
...
**Uniform Continuity Theorem.** If $$f:[a,b]\to\mathbb{R}$$ is continuous, then $$f$$ is uniformly continuous (for every $$\varepsilon>0$$, there exists $$\delta>0$$ such that for all $$x,t\in[a,b]$$, $$|x-t|<\delta$$ implies $$|f(x)-f(t)|<\varepsilon$$.
