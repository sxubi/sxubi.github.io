...
**Theorem (Supremum Property).** If a set $$S$$ has a supremum in $$\mathbb{R}$$ and $$\varepsilon>0$$, then there is $$x\in S$$ such that $$\sup S-\varepsilon<x\leq \sup S$$.

**Theorem (Infimum Property).** If a set $$S$$ has an infimum in $$\mathbb{R}$$ and $$\varepsilon>0$$, then there is $$x\in S$$ such that $$\inf S+\varepsilon>x\geq\inf S$$.
...
▲**Theorem (Limit Comparison Test).** Suppose $$f(x),g(x)>0$$ on $$(a,b]$$ and $$f,g\in L_\text{loc}((a,b])$$. If $$\displaystyle\lim_{x\to a^+}\frac{g(x)}{f(x)}$$ is a positive number $$L$$, then either both the improper integrals converge or both diverge. If $$\displaystyle\lim_{x\to a^+}\frac{g(x)}{f(x)}=0$$, then $$\int_a^b f(x)dx$$ converges implies $$\int_a^bg(x)dx$$ converges. If $$\displaystyle\lim_{x\to a^+}\frac{g(x)}{f(x)}=\infty$$, then $$\displaystyle\int_a^b f(x)dx$$ diverges implies $$\displaystyle\int_a^bg(x)dx$$ diverges. This is similar to $$[a,b)$$.
