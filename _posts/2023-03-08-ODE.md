...
**(2)** If $$M_y(x,y)\neq N_x(x,y)$$, the equation is inexact. Multiplying an integrating factor $$\mu(x,y)$$ to convert the equation into an exact one, with

$$(\mu M)_y=(\mu N)_x.$$

If $$\mu=\mu(x)$$, then $$\frac{d\mu}{dx}=\frac{M_y-N_x}{N}\mu$$.  
If $$\mu=\mu(y)$$, then $$\frac{d\mu}{dy}=\frac{M_x-N_y}{M}\mu$$.  
