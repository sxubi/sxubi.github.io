...
> *Proof.* 1. If $$K\leq G$$ has index 2, there are two possible cosets $$K$$ and $$G\backslash K$$. As $$eK=Ke$$ and cosets partition $$G$$, the other left coset and right coset must be $$G\backslash K.$$ So all left cosets and right cosets are the same. 2. Note that $$\forall a\in G, k\in K, aka^{-1}=k\in K$$.
...
> *Proof.* We have proved that $$K\triangleleft G$$. Define a group homomorphism $$\bar f:G/K\to\text{Im }f$$ with $$\bar f(aK)=f(a)$$. First we check it's well-defined: if $$a_1K=a_2K$$, then $$a_2^{-1}a_1\in K$$. Thus $$f(a_2)^{-1}f(a_1)=f(a_2^{-1}a_1)=e\Rightarrow f(a_1)=f(a_2),\bar f(a_1K)=\bar f(a_2K).$$     
> Now we check that it's a group homomorphism: $$\bar f(aKbK)=\bar f(abK)=f(ab)=f(a)f(b)=\bar f(aK)\bar f(bK).$$ Suppose $$\bar f(aK)=\bar f(bK)$$. Then $$f(a)=f(b)$$. Hence $$f(b^{-1}a)=e\Rightarrow aK=bK$$. Thus $$\bar f$$ is injective. By definition, $$\bar f$$ is surjective since $$\text{Im }\bar f=\text{Im }f$$. So $$\bar f$$ gives an isomorphism $$G/K\cong\text{Im f}\leq H$$. 
...
**Example.** 1. Trivial action: for any group $$G$$ acting on any set $$X$$, let $$\phi(g)=1_X$$ for all $$g$$.    
&emsp; 2. $$S_n$$ acts on $$\{1,\cdots,n\}$$ by permutation.    
&emsp; 3. $$D_{2n}$$ acts on the vertices of a regular $$n$$-gon.    
&emsp; 4. The rotations of a cube act on the faces/vertices/diagonals/axes of the cube.    
**Defintion** (Kernel of action)**.** The *kernel* of an action $$G$$ on $$X$$ is the kernel of $$\phi$$, i.e. all $$g$$ such that $$\phi(g)=1_X$$.    
Note that $$\text{ker}\phi\triangleleft G$$ and $$G/K$$ is isomorphism to a subgroup of $$\text{Sym}X$$, by the isomorphism theorem.    
...
> *proof.* Note that $$e(x)=x$$ by definition. Thus the stabiliser is not empty. Suppose $$g,h\in\text{Stab}(x)$$, then $$gh^{-1}(x)=g(h^{-1}(x))=g(x)=x$$. Thus $$gh^{-1}\in\text{Stab}(x)$$. 
...
> *Proof.* $$\forall x,x\in\text{Orb}(x)$$ as $$e(x)=x$$. Thus every $$x$$ is in some orbit. Suppose $$z\in\text{Orb}(x),z\in\text{Orb}(y)$$, we need to show $$\text{Orb}(x)=\text{Orb}(y)$$ (partition means that two orbits are either disjoint or the same). Know that $$z=g_1(x),z=g_2(x)$$ for some $$g_1,g_2$$. Then $$g_1(x)=g_2(y)\Rightarrow y=g_2^{-1}g_1(x).$$ For any $$w=g_3(y)\in\text{Orb}(y),w=g_3g_2^{-1}g_1(x)$$. Thus $$w\in\text{Orb}(x)$$. Thus $$\text{Orb}(y)\subseteq\text{Orb}(x)$$. Similarly, $$\text{Orb}(x)\subseteq\text{Orb}(y)$$. Thus $$\text{Orb}(x)=\text{Orb}(y).$$
...
> *Proof.* Prove by bijection: note that $$G:\text{Stab}(x)$$ is the set of cosets of $$\text{Stab}(x)$$. Define $$\theta:(G:\text{Stab}(x))\to\text{Orb}(x)$$ with $$g\text{Stab}(x)\mapsto g(x)$$. This is well-defined: if $$g\text{Stab}(x)=h\text{Stab}(x)$$, then $$h=gk$$ for some $$k\in\text{Stab}(x)$$. So $$h(x)=g(k(x))=g(x)$$. The map is surjective, since for any $$y\text{Orb}(x)$$, there is some $$g\in G$$, such that $$g(x)=y$$ by definition. Then $$\theta(g\text{Stab}(x))=y.$$ This is injective since if $$g(x)=h(x)$$ then $$h^{-1}g(x)=x$$. Thus $$h^{-1}g\in\text{Stab}(x)$$. Thus $$g\text{Stab}(x)=h\text{Stab}(x)$$. Hence the number of cosets is the same as the size of $$\text{Orb}(x)$$. Then consider the Lagrange's theorem.
...
> *Proof.* 1. $$\forall g\in G, x\in G, g(x)=gx\in G$$ by the definition of a group. 2. $$\forall x\in G, ex=x$$ by the definition of a group. 3. $$g(hx)=(gh)x$$ by associatvity. Thus it's an action. To show it faithful, need to show $$\forall x\in X, gx=x\Rightarrow g=e.$$ This follows the uniqueness of identity. To show transitivity, $$\forall x,y\in G,yx^{-1}(x)=y,$$ so any $$x$$ can be sent to any $$y$$.

**Theorem** (Cayley's theorem)**.** Every group is isomorphic to some subgroup of some symetric group.    

> *Proof.* Take the left regular action of $$G$$ on itself. This gives a group homorphism $$\phi:G\to \text{Sym }G$$ with $$\ker\phi=\{e\}$$ as the action is faithful. By the isomorphism theorem, $$G\cong\text{Im }\phi\leq\text{Sym }G$$.

**Lemma** (Left coset action)**.** Let $$H\leq G$$. Then $$G$$ acts on the left cosets of $$H$$ by left multiplication transitively.    

> *Proof.* First show it's an action: 1. $$g(aH)=(ga)H$$ is a coset of $$H$$. 2. $$e(aH)=(ea)H=aH.$$ 3. $$g_1(g_2(aH))=g_1g_2aH.$$ To show that it's transitive, given $$aH,bH,$$ note that $$(b^{-1}a)(aH)=bH.$$ So any $$aH$$ could be mapped to $$bH$$. 
...
> *Proof.* 1. $$g(x)=gxg^{-1}\in G$$. 2. $$e(x)=x.$$ 3. $$g(h(x))=ghxh^{-1}g^{-1}=(gh)x(gh)^{-1}=(gh)(x).$$
...
> *Proof.* Let $$K\triangleleft G$$. If $$k\in K$$, then for all $$g\in G, gkg^{-1}\in K$$. Thus $$\text{ccl}(k)\subseteq K$$. So $$K$$ is the union of the conjugacy classes of all its elements. Conversely, if $$K$$ is a union of conjugacy classes and a subgroup of $$G$$, then for all $$k\in K,g\in G,gkg^{-1}\in K.$$ Thus $$K$$ is normal.

**Lemma.** Let $$X$$ be the set of subgroups of $$G$$. Then $$G$$ acts by conjugation on $$X$$.    

> *Proof.* 1. If $$H\leq G$$, we need to show $$gHg^{-1}$$ is also a subgroup. Note that $$e\in gHg^{-1}$$, thus it's not empty. For any two elements $$gag^{-1}, gbg^{-1}$$, $$(gag^{-1})(gbg^{-1})^{-1}=g(ab^{-1})g^{-1}\in gHg^{-1}.$$ Thus $$gHg^{-1}$$ is also a subgroup. 2. $$eHe^{-1}=H$$. 3. $$g_1(g_2Hg_2^{-1})g_1^{-1}=(g_1g_2)H(g_1g_2)^{-1}.$$
...
<center><img src="http://sxubi.github.io/photos/ccl.png" width = "300"/></center>
...
> *Proof.* $$(\Rightarrow)$$ Let $$(r)$$ be a prime ideal. If $$r=0$$ then we are done. Otherwise, as prime ideals are proper (not the whole ring), $$r$$ is not a unit. Now suppose $$r$$ divides $$ab$$. Then $$ab\in (r)$$. But $$(r)$$ is prime. So $$a\in(r)$$ or $$b\in(r)$$. Thus $$r$$ divides $$a$$ or $$r$$ divides $$b$$. So $$r$$ is prime.     
...
> *Proof.* Suppose $$r=ab\in R$$. Since $$r$$ divides $$ab$$ and $$r$$ is prime, we have $$r$$ divides $$a$$ or $$r$$ divides $$b$$. Assume $$r$$ divdes $$a$$. Then $$a=rc$$ for some $$c\in R$$. Thus $$r=ab=rcb$$. Thus $$cb=1$$. So $$b$$ is a unit. Similar for $$r$$ divides $$b$$. 
...
> *Proof.* Let $$R$$ have a Euclidean function $$\phi:R\backslash\{0\}\to\mathbb{Z}_{\geq 0}$$. Let $$I\triangleleft R$$ be a non-zero ideal, and let $$b\in I\backslash\{0\}$$ be an element with $$\phi(b)$$ minimal. Then for any $$a\in I$$, $$a=bq+r$$, with $$r=0$$ or $$\phi(r)<\phi(b)$$. Note that $$r=a-bq\in I$$. Thus we cannot have $$\phi(r)<\phi(b)$$, and thus $$r=0$$. So $$a=bq\Rightarrow a\in(b)$$. Since this is true for all $$a\in I$$, $$I\subseteq (b)$$. Also since $$b\in I$$, we must have $$(b)\subseteq I$$. Thus $$I=(b)$$. This works for any ideal in $$R$$ and thus $$R$$ is a principal domain.
...
> *Proof.* Let $$p\in R$$ be irreducible, and suppose $$p$$ divides $$ab$$. Suppose $$p\nmid a$$, we need to show $$p$$ divides $$b$$. Consider the ideal $$(p,a)\triangleleft R$$. Since $$R$$ is a PID, we have $$(p,a)=(d)$$ for some $$d$$. Thus $$d$$ divides $$p$$ and $$d$$ divides $$a$$. Since $$d$$ divides $$p$$, $$p=q_1d$$ for some $$q_1$$. Since $$p$$ is irreducible, either $$q_1$$ or $$d$$ is a unit. If $$q_1$$ is a unit, then $$d=q_1^{-1}p$$ divides $$a$$. So $$a=q_1^{-1}px$$ for some $$x$$. This is a contradiction since $$p\nmid a$$. Thus $$d$$ is a unit. So $$(p,a)=(d)=R$$. Particularly, $$1_R\in (p,a)$$. Suppose $$1=rp+sa$$ for some $$r,s\in R$$. Then $$b=rpb+sab$$. Note that $$ab$$ is divisible by $$p$$, thus $$b$$ is divisible by $$p$$. So we're done. 
