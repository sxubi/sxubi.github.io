...
> * Remember to consider the **Mean Value Theorem**. e.g. $$\{a_n\}$$ is Cauchy sequence, $$b_n=\{\ln(1+a_n)\}$$ is also Cauchy sequence by considering $$f(x)=\ln(1+x)$$. The form becomes $$f'(c)|a_m-a_n|$$. Have to deal with $$f'(c)$$.
